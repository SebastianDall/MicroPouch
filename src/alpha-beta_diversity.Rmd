...
```{r}
gg_followup_data <- gg_alpha(metadata_with_alpha %>% mutate(remission = if_else(id == "pt023", "remission", remission)), x = c("Pre", "treatment_5","treatment_10","treatment_15","treatment_21", "Post"), alpha_metric = "richness")$data %>% 
  filter(id %in% c(paste0("pt0", c("01", "05", "06", "17", "23", "28"))))
...
gg_followup_data %>%
  ggplot(aes(x = x_axis, y = alpha_metric)) +
  geom_boxplot(outlier.shape = NA) +
  geom_line(data = filter(gg_followup_data, x_axis != "Donor"), aes(group = id, color = id), alpha = 0.6, position = position_dodge(0.2), linetype = "dashed") +
  geom_point(aes(group = id, color = id), position = position_dodge(0.2), size = 2) +
  geom_label(data = gg_followup_data_n, aes(label = label)) + 
  plot_theme +
  facet_grid(. ~ group) +
...
gg_followup_data <- gg_alpha(metadata_with_alpha, x = c("Pre", "Post", "followup_1m", "followup_3m"), alpha_metric = "shannon")$data %>% 
  filter(id %in% c(paste0("pt0", c("01", "05", "06", "17", "23", "28"))))
...
gg_followup_shannon <- gg_followup_data %>%
  ...
  geom_line(data = filter(gg_followup_data, x_axis != "Donor"), aes(group = id), alpha = 0.6, color = "grey60", position = position_dodge(0.2), linetype = "dashed") +
  geom_point(aes(group = id), position = position_dodge(0.2), size = 2, color = "grey80") +
  # geom_point(data = gg_followup_data_mean, color = "red", size = 5) +
  # geom_line(data = gg_followup_data_mean, aes(group = group), color = "red") +
  geom_label(data = gg_followup_data_n, aes(label = label)) + 
  plot_theme +
  facet_grid(. ~ group) +
  theme(legend.position = "none") +
