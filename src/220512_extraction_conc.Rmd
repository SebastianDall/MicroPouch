...
```{r}
files_lib <- list.files("../data/LIB_conc/", full.names = T, pattern = "*.xlsx")
```
...
```{r}
lib_conc_c <- lib_conc %>%
  mutate(rack_barcode = gsub("LIB","EXT",PlateID)) %>%
...
```{r}
ext_lib <- full_join(ext_conc_c, lib_conc_c, by = c("rack_barcode", "tube_row", "tube_column"))
```



```{r}
d_ext <- full_join(ext_link, ext_lib, by = c("rack_barcode", "tube_row", "tube_column"))
```

Creates a file with extraction and library concentrations
