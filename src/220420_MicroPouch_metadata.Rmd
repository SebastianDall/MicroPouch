...
```{r}
metadata_op <- read_delim("../data/metadata/MicroPouchOpen_DATA_2022-06-16_0816.csv", delim = ";") %>% 
  mutate(project = "OP") %>% 
  mutate(pdai_c1 = NA,
         pdai_c2 = NA,
         pdai_c3 = NA,
...
```{r}
metadata_np <- read_delim("../data/metadata/MicroPouchNP_DATA_2022-06-16_0817.csv", delim = ";") %>% 
  mutate(project = "NP") %>% 
  mutate(pdai_c1 = NA,
         pdai_c2 = NA,
         pdai_c3 = NA,
...
### Renaming columns
* selecting pouch data and uniting "duplicate columns
...
m_pdai <- m_p %>% 
  ...
  right_join(m_p, by = c("study_id", "redcap_event_name", "project")) %>% 
  arrange(study_id)
...
m_i <- m_v %>% 
  rename(
    ...
    defecations_per_day = pouchdys_q1,
    defecations_day = pouchdys_q2,
    defecations_night = pouchdys_q3,
    stool_consistency = pouchdys_q4,
    urge_to_defecate = pouchdys_q5,
    urgent_defecations_per_day = pouchdys_q6,
    hold_back_time = pouchdys_q7,
    leakages_per_day = pouchdys_q8,
    uncontrolled_defecations_per_day = pouchdys_q9,
    incomplete_defecations_per_day = pouchdys_q10,
    diaper = pouchdys_q11,
    anti_diarrhea_medicine = pouchdys_q12,
    antibiotics = pouchdys_q13,
    health_affect = pouchdys_q14
  )
```

### mutate and rename join columns

```{r}
m_tmp <- m_i %>% 
  mutate(study_id = as.character(study_id),
         study_id = if_else(nchar(study_id) == 1, true = paste0("pt00",study_id), false = paste0("pt0",study_id)),
         redcap_event_name = str_replace(redcap_event_name, "\\_arm\\_1", "")
  )
  recode(redcap_event_name,
         inclusion = inclusion_arm_1,
...
PaPr_c <- PaPr_overview %>%
  ...
  rename(inclusion = inklusion, 
         ...
         control_30dg = kontrol_30d,
         control_1mth = kontrol_1m,
         control_3mth = kontrol_3m,
         control_6mth = kontrol_6m,
...
```{r}
d_bar <- read_csv("../data/metadata/csv/sample_metadata.csv")
```
