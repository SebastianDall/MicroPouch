...
```{r}
metadata <- read_delim("../../../MicroPouch/data/metadata/MicroPouch_DATA_2022-06-21_1300.csv", delim = ";") %>% 
  mutate(project = "MP") %>% 
  fill(randomization_group)

metadata_op <- read_delim("../../../MicroPouch/data/metadata/MicroPouchOpen_DATA_2022-06-21_1303.csv", delim = ";") %>% 
...
metadata_np <- read_delim("../../../MicroPouch/data/metadata/MicroPouchNP_DATA_2022-06-21_1303.csv", delim = ";") %>% 
  mutate(project = "NP") %>% 
  mutate(pdai_c1 = NA,
         pdai_c2 = NA,
         pdai_c3 = NA,
...
m_sel <- m_all %>% 
  ...
  # Make cpdai_c# columns numeric
  mutate(across(starts_with("pdai_c"), .fns = ~if_else(. == "", NA_character_, .)))
...
m_pdai <- m_sel %>% 
  ...
  arrange(study_id) %>% 
  mutate(across(starts_with("pdai_c"), .fns = ~as.numeric(.)))
...
m_recode <- m_pdai %>% 
  mutate(
    ...
    diet_q12_09 = recode(diet_q12_09,
                        `1` = "daily",
                        `2` = "weekly",
                        `3` = "monthly",
                        `4` = "rarely"
    ...
    diet_q18_2 = recode(diet_q18_2,
        ...
        diet_q19 = recode(diet_q19,
                        `1` = "yes, daily",
                        `2` = "yes, not daily",
                        `3` = "no",
                        `4` = "don't know",
                        ),
        diet_q19a = recode(diet_q19a,
                        `1` = "yes",
                        `2` = "no",
                        `3` = "don't know"
                        ),
        diet_q19b = recode(diet_q19b,
                        `1` = "daily",
                        `2` = "less than daily",
                        `3` = "none",
                        `9` = "don't know"
...
m_rename <- m_recode %>% 
  rename(
    ...
    si_drowsy = sibdq_q1,
    si_cancel = sibdq_q2,
    si_activity_trouble = sibdq_q3,
    si_abdominal_pain = sibdq_q4,
    si_depression = sibdq_q5,
    si_gas = sibdq_q6,
    si_weight = sibdq_q7,
    si_relaxation = sibdq_q8,
    si_unn_defecation = sibdq_q9,
    si_anger = sibdq_q10,
    #diet
    di3_education = diet_q2,
    di8_spreadfat = diet_q8,
    di9_cookingfat = diet_q9,
    di11_1_vegetables = diet_q11_1,
    di12_9_energy_drink = diet_q12_09,
    di18_1_beer = diet_q18_1,
    di18_2_wine = diet_q18_2,
    di18_3_spirit = diet_q18_3,
    di19_smoking = diet_q19,
    di19_a_daily = diet_q19a,
    di19_b_lessdaily = diet_q19b
  )
```
...
```{r}
metadata_fmt <- read_dta("../../../MicroPouch/data/metadata/stata/metadata_fmt.dta")
```
...
d_fmt_no_NA <- d_fmt_renaming %>% 
  filter(if_any(.cols = 4:130,.fns = ~!is.na(.)))
```
...
### Load Barcodes
```{r}
barcode_metadata <- read_csv("../../../MicroPouch/data/metadata/csv/barcode_metadata.csv")
```
...
```{r}
DoBa_overview <- read_csv("../../../MicroPouch/data/metadata/csv/DoBa_oversigt.csv")
...
```{r}
PaDo_overview <- read_csv("../../../MicroPouch/data/metadata/MicroPouch patient donor batch oversigt april2022.csv")
...
```{r}
PaPr_overview <- read_csv("../../../MicroPouch/data/metadata/csv/PaPr_overview.csv")
...
```{r}
PaOP_overview <- read_csv("../../../MicroPouch/data/metadata/csv/PaOP_overview.csv")
...
```{r}
PaNP_overview <- read_csv("../../../MicroPouch/data/metadata/csv/PaNP_overview.csv")
...
```{r}
ext_conc <- read_csv("../../../MicroPouch/data/metagenomes_lab/220531_metagenome_libraries.csv")
